...
import { range, shuffle, round, random } from 'lodash';
...
export default ({
  ...
  rtm.on('message', async (message) => {
    ...
    // game開始処理
    if (message.text.match(/^alphabet0n$/)) {
      if (state.inGame) {
        assert(state.thread !== null);
        await slack.chat.postMessage({
        ...
        state.inGame = true;
        state.answer = wordlist[random(0, wordlist.length - 1)];
        const { ts } = await slack.chat.postMessage({
          text: stripIndent`
          Alphabet0n (${state.answer.length}桁) を開始します。
          スレッドにアルファベットでコールしてね`,
          channel: process.env.CHANNEL_SANDBOX as string,
          username: 'Alphabet0n',
          icon_emoji: 'abcd',
    ...
    if (message.thread_ts === state.thread) {
      assert(state.thread !== null);
      // call処理
      if (message.text.match(/^[a-zA-Z]+$/)) {
        if (!state.inGame) {
          return;
        }
        const call = message.text;

        if (call.length !== state.answer.length) {
          await slack.chat.postMessage({
            text: `文字数が違うよ:thinking_face: (${state.answer.length}桁)`,
            channel: process.env.CHANNEL_SANDBOX as string,
            username: 'Alphabet0n',
            icon_emoji: 'abcd',
            thread_ts: state.thread,
